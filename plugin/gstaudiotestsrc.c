...
gst_audio_test_src_do_seek (GstBaseSrc * basesrc, GstSegment * segment)
{
  ...
  src->next_sample =
      gst_util_uint64_scale_int_round (time, src->samplerate, GST_SECOND);
  ...
  if (GST_CLOCK_TIME_IS_VALID (segment->stop)) {
    time = segment->stop;
    src->sample_stop = gst_util_uint64_scale_int_round (time, src->samplerate,
        GST_SECOND);
...
gboolean
gst_audio_test_src_plugin_init (GstPlugin * plugin)
{
